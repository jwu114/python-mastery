...
def validated(func):
    ...
    my_annotations = dict(func.__annotations__)
    my_retcheck = my_annotations.pop('return', None)
    @wraps(func)
    def wrapper(*args, **kwargs):
        bound = my_signature.bind(*args, **kwargs)

        for name, val in my_annotations.items():
            val.check(bound.arguments[name])

        result = func(*args, **kwargs)

        if my_retcheck:
            my_retcheck.check(result)
